---
title: "2018.03_AM_HFO_2mo_growth_9d"
...
growth_2mo_9d_data <-juv_growth_data %>% 
  dplyr::filter(exp_time=="9 days") 

#filter for whether replicates should be included in model fitting
growth_2mo_9d_data <- growth_2mo_9d_data %>% 
  dplyr::filter(use_in_model=="Yes")



#filter for light treatment
growth_2mo_9d_PAR_data <- growth_2mo_9d_data %>% 
  dplyr::filter(light=="PAR")
  
growth_2mo_9d_UV_data <- growth_2mo_9d_data %>% 
  dplyr::filter(light=="UVR")

#check that filtering worked
View(growth_2mo_9d_PAR_data)
View(growth_2mo_9d_UV_data)
```


### Save filtered data as an RData object
```{r}
save(growth_2mo_9d_PAR_data, growth_2mo_9d_UV_data, file = "2018.03_AM_HFO_2mo_growth_9d_data.RData")
```
...
```{r}
load(file = "2018.03_AM_HFO_2mo_growth_9d_data.RData")

unique(growth_2mo_9d_PAR_data$raw.x)
par(mfrow=c(2,1))
hist(growth_2mo_9d_PAR_data$raw.x)
hist(growth_2mo_9d_PAR_data$growth)
par(mfrow=c(1,1))
plot(growth_2mo_9d_PAR_data$log.x, growth_2mo_9d_PAR_data$growth) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_growth_2mo_9d_PAR_all <- bayesnec::bnec(growth ~ crf(log.x, model = "decline"),
                                     data = growth_2mo_9d_PAR_data, family = Gamma(link = "identity"))

save(out_growth_2mo_9d_PAR_all, file = "2018.03_AM_HFO_2mo_growth_9d_PAR_modfit_all.RData")
beepr::beep("mario")
```
...
```{r}
rhat(out_growth_2mo_9d_PAR_all, rhat_cutoff = 1.03)
```


Check model summary
```{r}
out_growth_2mo_9d_PAR_all$mod_stats
summary(out_growth_2mo_9d_PAR_all)
```


Check the overall MANEC fit
```{r}
plot(out_growth_2mo_9d_PAR_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```

Check chain mixing, priors and the fit of the individual candidate models
```{r}
check_chains(out_growth_2mo_9d_PAR_all, filename = "2018.03_AM_HFO_2mo_growth_9d_PAR_modfit_chains")

check_priors(out_growth_2mo_9d_PAR_all, filename = "2018.03_AM_HFO_2mo_growth_9d_PAR_modfit_priors")

pdf("2018.03_AM_HFO_2mo_growth_9d_PAR_modfit_fitplots.pdf")
plot(out_growth_2mo_9d_PAR_all, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
...
```{r}
out_growth_2mo_9d_PAR_mod <- amend(out_growth_2mo_9d_PAR_all, drop = c("ecxexp"))

save(out_growth_2mo_9d_PAR_all, out_growth_2mo_9d_PAR_mod, file = "2018.03_AM_HFO_2mo_growth_9d_PAR_modfit_all.RData")
```


Compare the MANEC containing all OD candidate models to the amended MANEC
```{r}
plot(out_growth_2mo_9d_PAR_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
plot(out_growth_2mo_9d_PAR_mod, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
out_growth_2mo_9d_PAR <- out_growth_2mo_9d_PAR_mod
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out_growth_2mo_9d_PAR_NEC <- pull_out(out_growth_2mo_9d_PAR, model = c("nec"))
```


```{r}
plot(out_growth_2mo_9d_PAR_NEC, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out_growth_2mo_9d_PAR, out_growth_2mo_9d_PAR_NEC, file = "2018.03_AM_HFO_2mo_growth_9d_PAR_modfits_final.RData")
load("2018.03_AM_HFO_2mo_growth_9d_PAR_modfits_final.RData")
```
...
```{r}
NEC_growth_2mo_9d_PAR <- out_growth_2mo_9d_PAR_NEC$w_nec
NEC_growth_2mo_9d_PAR <- 10^NEC_growth_2mo_9d_PAR #if using log10(raw.x)
#NEC_growth_2mo_9d_PAR <- exp(NEC_growth_2mo_9d_PAR) #if using log(raw.x))
NEC_growth_2mo_9d_PAR
```

```{r}
EC10_growth_2mo_9d_PAR <- ecx(out_growth_2mo_9d_PAR, ecx_val = 10, type = "absolute")
EC10_growth_2mo_9d_PAR <- 10^EC10_growth_2mo_9d_PAR #if using log10(raw.x)
#EC10_growth_2mo_9d_PAR <- exp(EC10_growth_2mo_9d_PAR) #if using log(raw.x))
EC10_growth_2mo_9d_PAR
```

```{r}
EC50_growth_2mo_9d_PAR <- ecx(out_growth_2mo_9d_PAR, ecx_val = 50, type = "absolute")
EC50_growth_2mo_9d_PAR <- 10^EC50_growth_2mo_9d_PAR #if using log10(raw.x)
#EC50_growth_2mo_9d_PAR <- exp(EC50_growth_2mo_9d_PAR) #if using log(raw.x))
EC50_growth_2mo_9d_PAR
```
...
unique(growth_2mo_9d_UV_data$raw.x)
par(mfrow=c(2,1))
hist(growth_2mo_9d_UV_data$raw.x)
hist(growth_2mo_9d_UV_data$growth)
par(mfrow=c(1,1))
plot(growth_2mo_9d_UV_data$log.x, growth_2mo_9d_UV_data$growth) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_growth_2mo_9d_UV_all <- bayesnec::bnec(growth ~ crf(log.x, model = "decline"),
                                     data = growth_2mo_9d_UV_data, family = Gamma(link = "identity"))

save(out_growth_2mo_9d_UV_all, file = "2018.03_AM_HFO_2mo_growth_9d_UV_modfit_all.RData")
beepr::beep("mario")
```
...
Check if any candidate models failed the rHat test
```{r}
load(file = "2018.03_AM_HFO_2mo_growth_9d_UV_modfit_all.RData")
rhat(out_growth_2mo_9d_UV_all, rhat_cutoff = 1.03)
```


Check model summary
```{r}
out_growth_2mo_9d_UV_all$mod_stats
summary(out_growth_2mo_9d_UV_all)
```


Check the overall MANEC fit
```{r}
plot(out_growth_2mo_9d_UV_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```

Check chain mixing, priors and the fit of the individual candidate models
```{r}
check_chains(out_growth_2mo_9d_UV_all, filename = "2018.03_AM_HFO_2mo_growth_9d_UV_modfit_chains")

check_priors(out_growth_2mo_9d_UV_all, filename = "2018.03_AM_HFO_2mo_growth_9d_UV_modfit_priors")

pdf("2018.03_AM_HFO_2mo_growth_9d_UV_modfit_fitplots.pdf")
plot(out_growth_2mo_9d_UV_all, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
  ...
  *Any evidence of poor chain mixing or convergence issues?*
      ...
      - nec4param               unclear                   Minor-some evidence of poor chain mixing/instability
...
Drop inappropriate models and models with non-convergence or very poor chain mixing (if required)
```{r}
out_growth_2mo_9d_UV_mod <- amend(out_growth_2mo_9d_UV_all, drop = c("ecxexp"))

save(out_growth_2mo_9d_UV_all, out_growth_2mo_9d_UV_mod, file = "2018.03_AM_HFO_2mo_growth_9d_UV_modfit_all.RData")
```
...
```{r}
plot(out_growth_2mo_9d_UV_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
plot(out_growth_2mo_9d_UV_mod, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```
...
Rename model output for selected MANEC
```{r}
out_growth_2mo_9d_UV <- out_growth_2mo_9d_UV_mod
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out_growth_2mo_9d_UV_NEC <- pull_out(out_growth_2mo_9d_UV, model = c("nec"))
```


```{r}
plot(out_growth_2mo_9d_UV_NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out_growth_2mo_9d_UV, out_growth_2mo_9d_UV_NEC, file = "2018.03_AM_HFO_2mo_growth_9d_UV_modfits_final.RData")
load("2018.03_AM_HFO_2mo_growth_9d_UV_modfits_final.RData")
```
...
Extract NEC, EC10 and EC50 (with 95% credible intervals) from posterior and backtransform if required
```{r}
NEC_growth_2mo_9d_UV <- out_growth_2mo_9d_UV_NEC$w_nec
NEC_growth_2mo_9d_UV <- 10^NEC_growth_2mo_9d_UV #if using log10(raw.x)
#NEC_growth_2mo_9d_UV <- exp(NEC_growth_2mo_9d_UV) #if using log(raw.x))
NEC_growth_2mo_9d_UV
```

```{r}
EC10_growth_2mo_9d_UV <- ecx(out_growth_2mo_9d_UV, ecx_val = 10, type = "absolute")
EC10_growth_2mo_9d_UV <- 10^EC10_growth_2mo_9d_UV #if using log10(raw.x)
#EC10_growth_2mo_9d_UV <- exp(EC10_growth_2mo_9d_UV) #if using log(raw.x))
EC10_growth_2mo_9d_UV
```

```{r}
EC50_growth_2mo_9d_UV <- ecx(out_growth_2mo_9d_UV, ecx_val = 50, type = "absolute")
EC50_growth_2mo_9d_UV <- 10^EC50_growth_2mo_9d_UV #if using log10(raw.x)
#EC50_growth_2mo_9d_UV <- exp(EC50_growth_2mo_9d_UV) #if using log(raw.x))
EC50_growth_2mo_9d_UV
```
...
pred_growth_2mo_9d <- compare_posterior(x=list("PAR" = out_growth_2mo_9d_PAR, "UVR" = out_growth_2mo_9d_UV), comparison = "fitted", precision = 50)

pred_growth_2mo_9d_PAR <- pred_growth_2mo_9d$posterior_data %>% 
  dplyr::filter(model=="PAR")

pred_growth_2mo_9d_UV <- pred_growth_2mo_9d$posterior_data %>% 
  dplyr::filter(model=="UVR")



pred_growth_2mo_9d_NEC <- compare_posterior(list("PAR" = out_growth_2mo_9d_PAR_NEC, "UVR" = out_growth_2mo_9d_UV_NEC), comparison = "fitted", precision = 50)

pred_growth_2mo_9d_PAR_NEC <- pred_growth_2mo_9d_NEC$posterior_data %>% 
  dplyr::filter(model=="PAR")
pred_growth_2mo_9d_UV_NEC <- pred_growth_2mo_9d_NEC$posterior_data %>% 
 dplyr::filter(model=="UVR")


save(pred_growth_2mo_9d, pred_growth_2mo_9d_PAR, pred_growth_2mo_9d_UV, pred_growth_2mo_9d_PAR_NEC, pred_growth_2mo_9d_UV_NEC, file = "2018.03_AM_HFO_2mo_growth_9d_predvals.RData") # pred_growth_2mo_9d_NEC, pred_growth_2mo_9d_PAR_NEC, pred_growth_2mo_9d_UV_NEC <- add these back in once UV-section is working

load("2018.03_AM_HFO_2mo_growth_9d_predvals.RData")
str(pred_growth_2mo_9d_PAR)
str(pred_growth_2mo_9d_UV)
```
...
diff_growth_2mo_9d <- max(pred_growth_2mo_9d$prob_diff$prob)
diff_growth_2mo_9d
```
...
comp_growth_2mo_9dEC50 <- compare_posterior(x=list("PAR" = out_growth_2mo_9d_PAR, "UVR" = out_growth_2mo_9d_UV), comparison = "ecx", ecx_val = 50)
save(comp_growth_2mo_9dEC50, file = "2018.03_AM_HFO_2mo_growth_9d_posterior_comp_EC50.RData")
comp_growth_2mo_9dEC50$prob_diff
```
...
*Is EC50 +UVR lower than the EC50 -UVR?*
    - No. The -UVR EC50 is significantly lower than the +UVR EC50 (probability: 9.4%)
...
```{r}
load("2018.03_AM_HFO_2mo_growth_9d_data.RData")
load("2018.03_AM_HFO_2mo_growth_9d_predvals.RData")
...
p.all= p.all+ geom_point(data = growth_2mo_9d_UV_data, aes(x = raw.x, y = growth, colour=factor(light), shape = factor(light), alpha = 0.3), size = 2.5, position=position_jitter(width = .025, height=0.03))


p.all = p.all+ labs(x=expression(Concentration~(μg~TAH~"L"^{-1})), #Concentration~(μg~TAH~"L"^{-1})
              y=expression("9 d"~growth~("%")),
              subtitle="a)")
...
pred_growth_2mo_9d$prob_diff$inv.x <- 10^pred_growth_2mo_9d$prob_diff$x

p.comp.prob= ggplot(pred_growth_2mo_9d$prob_diff) +
...
plot_growth_2mo_9d_diff.prob <- p.comp.prob
plot_growth_2mo_9d_diff.prob
```
...
p.comp.EC50.diff = ggplot(comp_growth_2mo_9dEC50$diff_data, aes(x=diff)) +
...
#Rename & show plot
comp.plot_growth_2mo_9d_EC50.diff <- p.comp.EC50.diff
comp.plot_growth_2mo_9d_EC50.diff
```
...
```{r}
layout_HFO_2mo_growth_9d <- ggarrange(plot_growth_2mo_9d_all, comp.plot_growth_2mo_9d_EC50, plot_growth_2mo_9d_diff.est, plot_growth_2mo_9d_NEC,
          ncol = 3, nrow = 2,
          widths = c(0.5, 0.5))
layout_HFO_2mo_growth_9d
```
