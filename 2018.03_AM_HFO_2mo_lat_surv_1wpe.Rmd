---
title: "2018.03_AM_HFO_2mo_lat_surv_1wpe"
...
surv_2mo_1wpe_data <-juv_lat_surv_data %>% 
  dplyr::filter(exp_time=="1 week post-exposure") 

#filter for whether replicates should be included in model fitting
surv_2mo_1wpe_data <- surv_2mo_1wpe_data %>% 
  dplyr::filter(use_in_model=="Yes")



#filter for light treatment
surv_2mo_1wpe_PAR_data <- surv_2mo_1wpe_data %>% 
  dplyr::filter(light=="PAR")
  
surv_2mo_1wpe_UV_data <- surv_2mo_1wpe_data %>% 
  dplyr::filter(light=="UVR")

#check that filtering worked
View(surv_2mo_1wpe_PAR_data)
View(surv_2mo_1wpe_UV_data)
```


### Save filtered data as an RData object
```{r}
save(surv_2mo_1wpe_PAR_data, surv_2mo_1wpe_UV_data, file = "2018.03_AM_HFO_2mo_lat_surv_1wpe_data.RData")
```
...
```{r}
load(file = "2018.03_AM_HFO_2mo_lat_surv_1wpe_data.RData")

unique(surv_2mo_1wpe_PAR_data$raw.x)
par(mfrow=c(2,1))
hist(surv_2mo_1wpe_PAR_data$raw.x)
hist(surv_2mo_1wpe_PAR_data$surv)
par(mfrow=c(1,1))
plot(surv_2mo_1wpe_PAR_data$log.x, surv_2mo_1wpe_PAR_data$surv) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_lat_surv_2mo_1wpe_PAR_all <- bayesnec::bnec(suc|trials(tot) ~ crf(log.x, model = "decline"),
                                     data = surv_2mo_1wpe_PAR_data, family = beta_binomial2)

save(out_lat_surv_2mo_1wpe_PAR_all, file = "2018.03_AM_HFO_2mo_lat_surv_1wpe_PAR_modfit_all.RData")
beepr::beep("mario")
```
...
```{r}
rhat(out_lat_surv_2mo_1wpe_PAR_all, rhat_cutoff = 1.03)
```
...
Check model summary
```{r}
out_lat_surv_2mo_1wpe_PAR_all$mod_stats
summary(out_lat_surv_2mo_1wpe_PAR_all)
```


Check the overall MANEC fit
```{r}
plot(out_lat_surv_2mo_1wpe_PAR_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```

Check chain mixing, priors and the fit of the individual candidate models
```{r}
check_chains(out_lat_surv_2mo_1wpe_PAR_all, filename = "2018.03_AM_HFO_2mo_lat_surv_1wpe_PAR_modfit_chains")

check_priors(out_lat_surv_2mo_1wpe_PAR_all, filename = "2018.03_AM_HFO_2mo_lat_surv_1wpe_PAR_modfit_priors")

pdf("2018.03_AM_HFO_2mo_lat_surv_1wpe_PAR_modfit_fitplots.pdf")
plot(out_lat_surv_2mo_1wpe_PAR_all, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
  ...
  *Any evidence of poor chain mixing or convergence issues?*
      ...
      -  ecxwb2                   include                 Very minor evidence of poor chain mixing
...
```{r}
out_lat_surv_2mo_1wpe_PAR_mod <- amend(out_lat_surv_2mo_1wpe_PAR_all, drop = c("ecxexp"))

save(out_lat_surv_2mo_1wpe_PAR_all, out_lat_surv_2mo_1wpe_PAR_mod, file = "2018.03_AM_HFO_2mo_lat_surv_1wpe_PAR_modfit_all.RData")
```


Compare the MANEC containing all OD candidate models to the amended MANEC
```{r}
plot(out_lat_surv_2mo_1wpe_PAR_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
plot(out_lat_surv_2mo_1wpe_PAR_mod, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
out_lat_surv_2mo_1wpe_PAR <- out_lat_surv_2mo_1wpe_PAR_mod
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out_lat_surv_2mo_1wpe_PAR_NEC <- pull_out(out_lat_surv_2mo_1wpe_PAR, model = c("nec"))
```


```{r}
plot(out_lat_surv_2mo_1wpe_PAR_NEC, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out_lat_surv_2mo_1wpe_PAR, out_lat_surv_2mo_1wpe_PAR_NEC, file = "2018.03_AM_HFO_2mo_lat_surv_1wpe_PAR_modfits_final.RData")
load("2018.03_AM_HFO_2mo_lat_surv_1wpe_PAR_modfits_final.RData")
```
...
```{r}
NEC_lat_surv_2mo_1wpe_PAR <- out_lat_surv_2mo_1wpe_PAR_NEC$w_nec
NEC_lat_surv_2mo_1wpe_PAR <- 10^NEC_lat_surv_2mo_1wpe_PAR #if using log10(raw.x)
#NEC_lat_surv_2mo_1wpe_PAR <- exp(NEC_lat_surv_2mo_1wpe_PAR) #if using log(raw.x))
NEC_lat_surv_2mo_1wpe_PAR
```

```{r}
EC10_lat_surv_2mo_1wpe_PAR <- ecx(out_lat_surv_2mo_1wpe_PAR, ecx_val = 10, type = "absolute")
EC10_lat_surv_2mo_1wpe_PAR <- 10^EC10_lat_surv_2mo_1wpe_PAR #if using log10(raw.x)
#EC10_lat_surv_2mo_1wpe_PAR <- exp(EC10_lat_surv_2mo_1wpe_PAR) #if using log(raw.x))
EC10_lat_surv_2mo_1wpe_PAR
```

```{r}
EC50_lat_surv_2mo_1wpe_PAR <- ecx(out_lat_surv_2mo_1wpe_PAR, ecx_val = 50, type = "absolute")
EC50_lat_surv_2mo_1wpe_PAR <- 10^EC50_lat_surv_2mo_1wpe_PAR #if using log10(raw.x)
#EC50_lat_surv_2mo_1wpe_PAR <- exp(EC50_lat_surv_2mo_1wpe_PAR) #if using log(raw.x))
EC50_lat_surv_2mo_1wpe_PAR
```
...
unique(surv_2mo_1wpe_UV_data$raw.x)
par(mfrow=c(2,1))
hist(surv_2mo_1wpe_UV_data$raw.x)
hist(surv_2mo_1wpe_UV_data$surv)
par(mfrow=c(1,1))
plot(surv_2mo_1wpe_UV_data$log.x, surv_2mo_1wpe_UV_data$surv) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_lat_surv_2mo_1wpe_UV_all <- bayesnec::bnec(suc|trials(tot) ~ crf(log.x, model = "decline"),
                                     data = surv_2mo_1wpe_UV_data, family = beta_binomial2)

save(out_lat_surv_2mo_1wpe_UV_all, file = "2018.03_AM_HFO_2mo_lat_surv_1wpe_UV_modfit_all.RData")
beepr::beep("mario")
```
...
Check if any candidate models failed the rHat test
```{r}
rhat(out_lat_surv_2mo_1wpe_UV_all, rhat_cutoff = 1.03)
```


Drop candidate models that failed the rhat test
```{r}
out_lat_surv_2mo_1wpe_UV_mod <- amend(out_lat_surv_2mo_1wpe_UV_all, drop = c("nec3param", "ecxll5", "ecxll4", "ecxll3"))
save(out_lat_surv_2mo_1wpe_UV_all, out_lat_surv_2mo_1wpe_UV_mod, file = "2018.03_AM_HFO_2mo_lat_surv_1wpe_UV_modfit_all.RData")
```


Check model summary
```{r}
out_lat_surv_2mo_1wpe_UV_mod$mod_stats
summary(out_lat_surv_2mo_1wpe_UV_mod)
```


Check the overall MANEC fit
```{r}
plot(out_lat_surv_2mo_1wpe_UV_mod, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```

Check chain mixing, priors and the fit of the individual candidate models
```{r}
check_chains(out_lat_surv_2mo_1wpe_UV_mod, filename = "2018.03_AM_HFO_2mo_lat_surv_1wpe_UV_modfit_chains")

check_priors(out_lat_surv_2mo_1wpe_UV_mod, filename = "2018.03_AM_HFO_2mo_lat_surv_1wpe_UV_modfit_priors")

pdf("2018.03_AM_HFO_2mo_lat_surv_1wpe_UV_modfit_fitplots.pdf")
plot(out_lat_surv_2mo_1wpe_UV_mod, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
  ...
  *Any evidence of poor chain mixing or convergence issues?*
      ...
      - nec4param               exclude                   Evidence of por chain mixing
      - ecx4param               include                   Minor-some evidence of poor chain mixing
...
Drop inappropriate models and models with non-convergence or very poor chain mixing (if required)
```{r}
out_lat_surv_2mo_1wpe_UV_mod <- amend(out_lat_surv_2mo_1wpe_UV_mod, drop = c("nec4param","ecxexp"))
...
```{r}
plot(out_lat_surv_2mo_1wpe_UV_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
plot(out_lat_surv_2mo_1wpe_UV_mod, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```
...
Rename model output for selected MANEC
```{r}
out_lat_surv_2mo_1wpe_UV <- out_lat_surv_2mo_1wpe_UV_mod
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out_lat_surv_2mo_1wpe_UV_NEC <- pull_out(out_lat_surv_2mo_1wpe_UV, model = c("nec"))
```


```{r}
plot(out_lat_surv_2mo_1wpe_UV_NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out_lat_surv_2mo_1wpe_UV, out_lat_surv_2mo_1wpe_UV_NEC, file = "2018.03_AM_HFO_2mo_lat_surv_1wpe_UV_modfits_final.RData")
load("2018.03_AM_HFO_2mo_lat_surv_1wpe_UV_modfits_final.RData")
```
...
Extract NEC, EC10 and EC50 (with 95% credible intervals) from posterior and backtransform if required
      ...
      No NEC model subset available

```{r}
EC10_lat_surv_2mo_1wpe_UV <- ecx(out_lat_surv_2mo_1wpe_UV, ecx_val = 10, type = "absolute")
EC10_lat_surv_2mo_1wpe_UV <- 10^EC10_lat_surv_2mo_1wpe_UV #if using log10(raw.x)
#EC10_lat_surv_2mo_1wpe_UV <- exp(EC10_lat_surv_2mo_1wpe_UV) #if using log(raw.x))
EC10_lat_surv_2mo_1wpe_UV
```

```{r}
EC50_lat_surv_2mo_1wpe_UV <- ecx(out_lat_surv_2mo_1wpe_UV, ecx_val = 50, type = "absolute")
EC50_lat_surv_2mo_1wpe_UV <- 10^EC50_lat_surv_2mo_1wpe_UV #if using log10(raw.x)
#EC50_lat_surv_2mo_1wpe_UV <- exp(EC50_lat_surv_2mo_1wpe_UV) #if using log(raw.x))
EC50_lat_surv_2mo_1wpe_UV
```
...
pred_lat_surv_2mo_1wpe <- compare_posterior(x=list("PAR" = out_lat_surv_2mo_1wpe_PAR, "UVR" = out_lat_surv_2mo_1wpe_UV), comparison = "fitted", precision = 50)

pred_lat_surv_2mo_1wpe_PAR <- pred_lat_surv_2mo_1wpe$posterior_data %>% 
  dplyr::filter(model=="PAR")

pred_lat_surv_2mo_1wpe_UV <- pred_lat_surv_2mo_1wpe$posterior_data %>% 
  dplyr::filter(model=="UVR")



pred_lat_surv_2mo_1wpe_NEC <- compare_posterior(list("PAR" = out_lat_surv_2mo_1wpe_PAR_NEC, "UVR" = out_lat_surv_2mo_1wpe_UV_NEC), comparison = "fitted", precision = 50)

pred_lat_surv_2mo_1wpe_PAR_NEC <- pred_lat_surv_2mo_1wpe_NEC$posterior_data %>% 
  dplyr::filter(model=="PAR")
pred_lat_surv_2mo_1wpe_UV_NEC <- pred_lat_surv_2mo_1wpe_NEC$posterior_data %>% 
 dplyr::filter(model=="UVR")


save(pred_lat_surv_2mo_1wpe, pred_lat_surv_2mo_1wpe_PAR, pred_lat_surv_2mo_1wpe_UV, pred_lat_surv_2mo_1wpe_PAR_NEC, pred_lat_surv_2mo_1wpe_UV_NEC, file = "2018.03_AM_HFO_2mo_lat_surv_1wpe_predvals.RData") # pred_lat_surv_2mo_1wpe_NEC, pred_lat_surv_2mo_1wpe_PAR_NEC, pred_lat_surv_2mo_1wpe_UV_NEC <- add these back in once UV-section is working

load("2018.03_AM_HFO_2mo_lat_surv_1wpe_predvals.RData")
str(pred_lat_surv_2mo_1wpe_PAR)
str(pred_lat_surv_2mo_1wpe_UV)
```
...
diff_lat_surv_2mo_1wpe <- max(pred_lat_surv_2mo_1wpe$prob_diff$prob)
diff_lat_surv_2mo_1wpe
```
...
comp_lat_surv_2mo_1wpe_EC50 <- compare_posterior(x=list("PAR" = out_lat_surv_2mo_1wpe_PAR, "UVR" = out_lat_surv_2mo_1wpe_UV), comparison = "ecx", ecx_val = 50)
save(comp_lat_surv_2mo_1wpe_EC50, file = "2018.03_AM_HFO_2mo_lat_surv_1wpe_posterior_comp_EC50.RData")
comp_lat_surv_2mo_1wpe_EC50$prob_diff
```
...
```{r}
load("2018.03_AM_HFO_2mo_lat_surv_1wpe_data.RData")
load("2018.03_AM_HFO_2mo_lat_surv_1wpe_predvals.RData")
...
p.all= p.all+ geom_point(data = surv_2mo_1wpe_UV_data, aes(x = raw.x, y = surv, colour=factor(light), shape = factor(light), alpha = 0.3), size = 2.5, position=position_jitter(width = .025, height=0.03))
...
p_NEC= p_NEC+ geom_point(data = surv_2mo_1wpe_UV_data, aes(x = raw.x, y = surv, colour=factor(light), shape = factor(light), alpha = 0.3), size = 2.5, position=position_jitter(width = .025, height=0.03))
...
pred_lat_surv_2mo_1wpe$prob_diff$inv.x <- 10^pred_lat_surv_2mo_1wpe$prob_diff$x

p.comp.prob= ggplot(pred_lat_surv_2mo_1wpe$prob_diff) +
...
plot_lat_surv_2mo_1wpe_diff.prob <- p.comp.prob
plot_lat_surv_2mo_1wpe_diff.prob
```
...
p.comp.EC50.diff = ggplot(comp_lat_surv_2mo_1wpe_EC50$diff_data, aes(x=diff)) +
...
#Rename & show plot
comp.plot_lat_surv_2mo_1wpe_EC50.diff <- p.comp.EC50.diff
comp.plot_lat_surv_2mo_1wpe_EC50.diff
```
...
```{r}
layout_HFO_2mo_lat_surv_1wpe <- ggarrange(plot_lat_surv_2mo_1wpe_all, comp.plot_lat_surv_2mo_1wpe_EC50, plot_lat_surv_2mo_1wpe_diff.est, plot_lat_surv_2mo_1wpe_NEC,
          ncol = 3, nrow = 2,
          widths = c(0.5, 0.5))
layout_HFO_2mo_lat_surv_1wpe
```
