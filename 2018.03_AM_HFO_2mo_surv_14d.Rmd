---
title: "2018.03_AM_HFO_2mo_surv_14d"
...
juv_surv_data <- readr::read_csv("csv files/2018.03_AM_HFO_2mo_14d_surv.csv") %>% 
  data.frame() %>% dplyr::mutate(light=as.factor(as.character(light)),
                                 ...
                                 tot=as.integer(20)) #to account for the 5% variability between repeat area measurements
str(juv_surv_data)
head(juv_surv_data)
tail(juv_surv_data)
```
...
surv_2mo_14d_data <-juv_surv_data %>% 
  dplyr::filter(exp_time=="14 days") 

#filter for whether replicates should be included in model fitting
surv_2mo_14d_data <- surv_2mo_14d_data %>% 
  dplyr::filter(use_in_model=="Yes")



#filter for light treatment
surv_2mo_14d_PAR_data <- surv_2mo_14d_data %>% 
  dplyr::filter(light=="PAR")
  
surv_2mo_14d_UV_data <- surv_2mo_14d_data %>% 
  dplyr::filter(light=="UVR")

#check that filtering worked
View(surv_2mo_14d_PAR_data)
View(surv_2mo_14d_UV_data)
```


### Save filtered data as an RData object
```{r}
save(surv_2mo_14d_PAR_data, surv_2mo_14d_UV_data, file = "2018.03_AM_HFO_2mo_surv_14d_data.RData")
```
...
```{r}
load(file = "2018.03_AM_HFO_2mo_surv_14d_data.RData")

unique(surv_2mo_14d_PAR_data$raw.x)
par(mfrow=c(2,1))
hist(surv_2mo_14d_PAR_data$raw.x)
hist(surv_2mo_14d_PAR_data$surv)
par(mfrow=c(1,1))
plot(surv_2mo_14d_PAR_data$log.x, surv_2mo_14d_PAR_data$surv) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_surv_2mo_14d_PAR_all <- bayesnec::bnec(suc|trials(tot) ~ crf(log.x, model = "decline"),
                                     data = surv_2mo_14d_PAR_data, family = beta_binomial2)

save(out_surv_2mo_14d_PAR_all, file = "2018.03_AM_HFO_2mo_surv_14d_PAR_modfit_all.RData")
beepr::beep("mario")
```
...
```{r}
rhat(out_surv_2mo_14d_PAR_all, rhat_cutoff = 1.03)
```

All candidate models passed the Rhat test

Check model summary
```{r}
out_surv_2mo_14d_PAR_all$mod_stats
summary(out_surv_2mo_14d_PAR_all)
```


Check the overall MANEC fit
```{r}
plot(out_surv_2mo_14d_PAR_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```

Check chain mixing, priors and the fit of the individual candidate models
```{r}
check_chains(out_surv_2mo_14d_PAR_all, filename = "2018.03_AM_HFO_2mo_surv_14d_PAR_modfit_chains")

check_priors(out_surv_2mo_14d_PAR_all, filename = "2018.03_AM_HFO_2mo_surv_14d_PAR_modfit_priors")

pdf("2018.03_AM_HFO_2mo_surv_14d_PAR_modfit_fitplots.pdf")
plot(out_surv_2mo_14d_PAR_all, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
...
```{r}
out_surv_2mo_14d_PAR_mod <- amend(out_surv_2mo_14d_PAR_all, drop = c("ecxexp"))

save(out_surv_2mo_14d_PAR_all, out_surv_2mo_14d_PAR_mod, file = "2018.03_AM_HFO_2mo_surv_14d_PAR_modfit_all.RData")
```
...
```{r}
plot(out_surv_2mo_14d_PAR_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
plot(out_surv_2mo_14d_PAR_mod, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
out_surv_2mo_14d_PAR <- out_surv_2mo_14d_PAR_mod
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out_surv_2mo_14d_PAR_NEC <- pull_out(out_surv_2mo_14d_PAR, model = c("nec"))
```


```{r}
plot(out_surv_2mo_14d_PAR_NEC, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out_surv_2mo_14d_PAR, out_surv_2mo_14d_PAR_NEC, file = "2018.03_AM_HFO_2mo_surv_14d_PAR_modfits_final.RData")
load("2018.03_AM_HFO_2mo_surv_14d_PAR_modfits_final.RData")
```
...
```{r}
NEC_surv_2mo_14d_PAR <- out_surv_2mo_14d_PAR_NEC$w_nec
NEC_surv_2mo_14d_PAR <- 10^NEC_surv_2mo_14d_PAR #if using log10(raw.x)
#NEC_surv_2mo_14d_PAR <- exp(NEC_surv_2mo_14d_PAR) #if using log(raw.x))
NEC_surv_2mo_14d_PAR
```

```{r}
EC10_surv_2mo_14d_PAR <- ecx(out_surv_2mo_14d_PAR, ecx_val = 10, type = "absolute")
EC10_surv_2mo_14d_PAR <- 10^EC10_surv_2mo_14d_PAR #if using log10(raw.x)
#EC10_surv_2mo_14d_PAR <- exp(EC10_surv_2mo_14d_PAR) #if using log(raw.x))
EC10_surv_2mo_14d_PAR
```

```{r}
EC50_surv_2mo_14d_PAR <- ecx(out_surv_2mo_14d_PAR, ecx_val = 50, type = "absolute")
EC50_surv_2mo_14d_PAR <- 10^EC50_surv_2mo_14d_PAR #if using log10(raw.x)
#EC50_surv_2mo_14d_PAR <- exp(EC50_surv_2mo_14d_PAR) #if using log(raw.x))
EC50_surv_2mo_14d_PAR
```
...
tail(surv_2mo_14d_UV_data)
unique(surv_2mo_14d_UV_data$raw.x)
par(mfrow=c(2,1))
hist(surv_2mo_14d_UV_data$raw.x)
hist(surv_2mo_14d_UV_data$surv)
par(mfrow=c(1,1))
plot(surv_2mo_14d_UV_data$log.x, surv_2mo_14d_UV_data$surv) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_surv_2mo_14d_UV_all <- bayesnec::bnec(suc|trials(tot) ~ crf(log.x, model = "decline"),
                                     data = surv_2mo_14d_UV_data, family = beta_binomial2)

save(out_surv_2mo_14d_UV_all, file = "2018.03_AM_HFO_2mo_surv_14d_UV_modfit_all.RData")
beepr::beep("mario")
```
...
Check if any candidate models failed the rHat test
```{r}
rhat(out_surv_2mo_14d_UV_all, rhat_cutoff = 1.03)
```


Drop candidate models that failed the rhat test
```{r}
out_surv_2mo_14d_UV_mod <- amend(out_surv_2mo_14d_UV_all, drop = c("ecxll5"))
save(out_surv_2mo_14d_UV_all, out_surv_2mo_14d_UV_mod, file = "2018.03_AM_HFO_2mo_surv_14d_UV_modfit_all.RData")
```


Check model summary
```{r}
out_surv_2mo_14d_UV_mod$mod_stats
summary(out_surv_2mo_14d_UV_mod)
```


Check the overall MANEC fit
```{r}
plot(out_surv_2mo_14d_UV_mod, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```


Check chain mixing, priors and the fit of the individual candidate models
```{r}
check_chains(out_surv_2mo_14d_UV_mod, filename = "2018.03_AM_HFO_2mo_surv_14d_UV_modfit_chains")

check_priors(out_surv_2mo_14d_UV_mod, filename = "2018.03_AM_HFO_2mo_surv_14d_UV_modfit_priors")

pdf("2018.03_AM_HFO_2mo_surv_14d_UV_modfit_fitplots.pdf")
plot(out_surv_2mo_14d_UV_mod, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
  ...
  *Any evidence of poor chain mixing or convergence issues?*
      ...
      - nec4param                   include               Minor evidence of poor chain mixing.
      - ecxwb2                      exclude               Section of poor chain mixing (1 chain)
...
Drop inappropriate models and models with non-convergence or very poor chain mixing (if required)
```{r}
out_surv_2mo_14d_UV_mod <- amend(out_surv_2mo_14d_UV_mod, drop = c("ecxwb2", "ecxll3", "ecxexp"))
...
```{r}
plot(out_surv_2mo_14d_UV_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
plot(out_surv_2mo_14d_UV_mod, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```
...
```{r}
out_surv_2mo_14d_UV <- out_surv_2mo_14d_UV_mod
```


Extract the NEC-model subset for use in extraction of NEC and comUVisons of posterior distributions
```{r}
out_surv_2mo_14d_UV_NEC <- pull_out(out_surv_2mo_14d_UV, model = c("nec"))
```


```{r}
plot(out_surv_2mo_14d_UV_NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out_surv_2mo_14d_UV, out_surv_2mo_14d_UV_NEC, file = "2018.03_AM_HFO_2mo_surv_14d_UV_modfits_final.RData")
load("2018.03_AM_HFO_2mo_surv_14d_UV_modfits_final.RData")
```
...
Extract NEC, EC10 and EC50 (with 95% credible intervals) from posterior and backtransform if required
```{r}
NEC_surv_2mo_14d_UV <- out_surv_2mo_14d_UV_NEC$w_nec
NEC_surv_2mo_14d_UV <- 10^NEC_surv_2mo_14d_UV #if using log10(raw.x)
#NEC_surv_2mo_14d_UV <- exp(NEC_surv_2mo_14d_UV) #if using log(raw.x))
NEC_surv_2mo_14d_UV
```

```{r}
EC10_surv_2mo_14d_UV <- ecx(out_surv_2mo_14d_UV, ecx_val = 10, type = "absolute")
EC10_surv_2mo_14d_UV <- 10^EC10_surv_2mo_14d_UV #if using log10(raw.x)
#EC10_surv_2mo_14d_UV <- exp(EC10_surv_2mo_14d_UV) #if using log(raw.x))
EC10_surv_2mo_14d_UV
```

```{r}
EC50_surv_2mo_14d_UV <- ecx(out_surv_2mo_14d_UV, ecx_val = 50, type = "absolute")
EC50_surv_2mo_14d_UV <- 10^EC50_surv_2mo_14d_UV #if using log10(raw.x)
#EC50_surv_2mo_14d_UV <- exp(EC50_surv_2mo_14d_UV) #if using log(raw.x))
EC50_surv_2mo_14d_UV
```
...
pred_surv_2mo_14d <- compare_posterior(x=list("PAR" = out_surv_2mo_14d_PAR, "UVR" = out_surv_2mo_14d_UV), comparison = "fitted", precision = 50)

pred_surv_2mo_14d_PAR <- pred_surv_2mo_14d$posterior_data %>% 
  dplyr::filter(model=="PAR")

pred_surv_2mo_14d_UV <- pred_surv_2mo_14d$posterior_data %>% 
  dplyr::filter(model=="UVR")



pred_surv_2mo_14d_NEC <- compare_posterior(list("PAR" = out_surv_2mo_14d_PAR_NEC, "UVR" = out_surv_2mo_14d_UV_NEC), comparison = "fitted", precision = 50)

pred_surv_2mo_14d_PAR_NEC <- pred_surv_2mo_14d_NEC$posterior_data %>% 
  dplyr::filter(model=="PAR")
pred_surv_2mo_14d_UV_NEC <- pred_surv_2mo_14d_NEC$posterior_data %>% 
 dplyr::filter(model=="UVR")


save(pred_surv_2mo_14d, pred_surv_2mo_14d_PAR, pred_surv_2mo_14d_UV, pred_surv_2mo_14d_PAR_NEC, pred_surv_2mo_14d_UV_NEC, file = "2018.03_AM_HFO_2mo_surv_14d_predvals.RData") # pred_surv_2mo_14d_NEC, pred_surv_2mo_14d_PAR_NEC, pred_surv_2mo_14d_UV_NEC <- add these back in once UV-section is working

load("2018.03_AM_HFO_2mo_surv_14d_predvals.RData")
str(pred_surv_2mo_14d_PAR)
str(pred_surv_2mo_14d_UV)
```

## Check the median repsonse for controls and unaffected concentrations
```{r}
head(pred_surv_2mo_14d_PAR)
head(pred_surv_2mo_14d_UV)
```
...
diff_surv_2mo_14d <- max(pred_surv_2mo_14d$prob_diff$prob)
diff_surv_2mo_14d
```
...
comp_surv_2mo_14dEC50 <- compare_posterior(x=list("PAR" = out_surv_2mo_14d_PAR, "UVR" = out_surv_2mo_14d_UV), comparison = "ecx", ecx_val = 50)
save(comp_surv_2mo_14dEC50, file = "2018.03_AM_HFO_2mo_surv_14d_posterior_comp_EC50.RData")
comp_surv_2mo_14dEC50$prob_diff
```
...
*Is EC50 +UVR lower than the EC50 -UVR?*
    - Possibly, with a likelihood of 89.58%. 
...
```{r}
load("2018.03_AM_HFO_2mo_surv_14d_data.RData")
load("2018.03_AM_HFO_2mo_surv_14d_predvals.RData")
...
p.all= p.all+ geom_point(data = surv_2mo_14d_UV_data, aes(x = raw.x, y = surv, colour=factor(light), shape = factor(light), alpha = 0.3), size = 2.5, position=position_jitter(width = .025, height=0.03))
...
p.all= p.all+ scale_x_continuous(limits = c(0.09, 500), trans="log10", breaks = c(0.01, 1, 10, 100), labels = c(0, 1, 10, 100))
...
#Rename & show plot
plot_surv_2mo_14d_NEC <- p_NEC
plot_surv_2mo_14d_NEC
```
...
pred_surv_2mo_14d$prob_diff$inv.x <- 10^pred_surv_2mo_14d$prob_diff$x

p.comp.prob= ggplot(pred_surv_2mo_14d$prob_diff) +
...
plot_surv_2mo_14d_diff.prob <- p.comp.prob
plot_surv_2mo_14d_diff.prob
```
...
p.comp.EC50.diff = ggplot(comp_surv_2mo_14dEC50$diff_data, aes(x=diff)) +
...
#Rename & show plot
comp.plot_surv_2mo_14d_EC50.diff <- p.comp.EC50.diff
comp.plot_surv_2mo_14d_EC50.diff
```
...
```{r}
layout_2mo_surv_14d_layout <- ggarrange(plot_surv_2mo_14d_all, comp.plot_surv_2mo_14d_EC50, plot_surv_2mo_14d_diff.est, plot_surv_2mo_14d_NEC,
          ncol = 3, nrow = 2,
          widths = c(0.5, 0.5))
layout_2mo_surv_14d_layout
```
