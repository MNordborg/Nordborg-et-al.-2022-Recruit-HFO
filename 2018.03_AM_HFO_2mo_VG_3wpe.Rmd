---
title: "2018.03_AM_HFO_2mo_lat_VG_3wpe"
...
VG_2mo_3wpe_data <-juv_lat_VG_data %>% 
  dplyr::filter(exp_time=="3 weeks post-exposure") 

#filter for whether replicates should be included in model fitting
VG_2mo_3wpe_data <- VG_2mo_3wpe_data %>% 
  dplyr::filter(use_in_model=="Yes")



#filter for light treatment
VG_2mo_3wpe_PAR_data <- VG_2mo_3wpe_data %>% 
  dplyr::filter(light=="PAR")
  
VG_2mo_3wpe_UV_data <- VG_2mo_3wpe_data %>% 
  dplyr::filter(light=="UVR")

#check that filtering worked
View(VG_2mo_3wpe_PAR_data)
View(VG_2mo_3wpe_UV_data)
```


### Save filtered data as an RData object
```{r}
save(VG_2mo_3wpe_PAR_data, VG_2mo_3wpe_UV_data, file = "2018.03_AM_HFO_2mo_lat_VG_3wpe_data.RData")
```
...
```{r}
load(file = "2018.03_AM_HFO_2mo_lat_VG_3wpe_data.RData")

unique(VG_2mo_3wpe_PAR_data$raw.x)
par(mfrow=c(2,1))
hist(VG_2mo_3wpe_PAR_data$raw.x)
hist(VG_2mo_3wpe_PAR_data$VG)
par(mfrow=c(1,1))
plot(VG_2mo_3wpe_PAR_data$log.x, VG_2mo_3wpe_PAR_data$VG) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_lat_VG_2mo_3wpe_PAR_all <- bayesnec::bnec(VG ~ crf(log.x, model = "decline"),
                                     data = VG_2mo_3wpe_PAR_data, family = bernoulli(link = "identity"))
save(out_lat_VG_2mo_3wpe_PAR_all, file = "2018.03_AM_HFO_2mo_lat_VG_3wpe_PAR_modfit_all.RData")

beepr::beep("mario")
```

    - No models fitted as no decline in vertical growth  with increasing treatment concentration observed.
...
unique(VG_2mo_3wpe_UV_data$raw.x)
par(mfrow=c(2,1))
hist(VG_2mo_3wpe_UV_data$raw.x)
hist(VG_2mo_3wpe_UV_data$VG)
par(mfrow=c(1,1))
plot(VG_2mo_3wpe_UV_data$log.x, VG_2mo_3wpe_UV_data$VG) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_lat_VG_2mo_3wpe_UV_all <- bayesnec::bnec(VG ~ crf(log.x, model = "decline"),
                                     data = VG_2mo_3wpe_UV_data, family = bernoulli(link = "identity"))
save(out_lat_VG_2mo_3wpe_UV_all, file = "2018.03_AM_HFO_2mo_lat_VG_3wpe_UV_modfit_all.RData")
...
```{r}
load("2018.03_AM_HFO_2mo_lat_VG_3wpe_data.RData")
...
p.all= p.all+ stat_dots(data = VG_2mo_3wpe_PAR_data, aes(x = raw.x, y = VG, colour=factor(light), shape = factor(light), alpha = 0.3, side = ifelse(VG == 0, "top", "bottom")), scale = 0.6, position=position_jitter(width = .07, height=0))
p.all= p.all+ stat_dots(data = VG_2mo_3wpe_UV_data, aes(x = raw.x, y = VG, colour=factor(light), shape = factor(light), alpha = 0.3, side = ifelse(VG == 0, "top", "bottom")), scale = 0.6, position=position_jitter(width = .07, height=0))
...
#p.all= p.all+ geom_point(data = VG_2mo_3wpe_UV_data, aes(x = raw.x, y = VG, colour=factor(light), shape = factor(light), alpha = 0.3), size = 2.5, position=position_jitter(width = .025, height=0.03))
