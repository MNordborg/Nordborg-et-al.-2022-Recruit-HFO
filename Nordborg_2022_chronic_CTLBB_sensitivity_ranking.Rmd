...
# Load the McGrath et al.(2018) chronic CTLBB database (36spp) and perform initial data wrangling
```{r}
pch.vec <- c(17, 2, 15, 1)

data <- read.csv("McGrath2018_chronic_CTLBBs.csv") %>%
  data.frame() %>%
...
# Derive percentile sensitivity values for back-calculated CTLBB values
```{r}
...
```{r}
write.csv(MK.pa, file="Ch2_Estimated_percent_values_chronic.csv")
```
