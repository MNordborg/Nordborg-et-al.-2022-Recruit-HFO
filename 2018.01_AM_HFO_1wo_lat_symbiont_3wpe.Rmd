---
title: "2018.01_AM_HFO_1wo_lat_symb_3wpe"
...
symb_1wo_3wpe_data <-juv_lat_symb_data %>% 
  dplyr::filter(exp_time=="3 weeks post-exposure") 

#filter for whether replicates should be included in model fitting
symb_1wo_3wpe_data <- symb_1wo_3wpe_data %>% 
  dplyr::filter(use_in_model=="Yes")



#filter for light treatment
symb_1wo_3wpe_PAR_data <- symb_1wo_3wpe_data %>% 
  dplyr::filter(light=="PAR")
  
symb_1wo_3wpe_UV_data <- symb_1wo_3wpe_data %>% 
  dplyr::filter(light=="UVR")

#check that filtering worked
View(symb_1wo_3wpe_PAR_data)
View(symb_1wo_3wpe_UV_data)
```


### Save filtered data as an RData object
```{r}
save(symb_1wo_3wpe_PAR_data, symb_1wo_3wpe_UV_data, file = "2018.01_AM_HFO_1wo_lat_symb_3wpe_data.RData")
```
...
```{r}
load(file = "2018.01_AM_HFO_1wo_lat_symb_3wpe_data.RData")

unique(symb_1wo_3wpe_PAR_data$raw.x)
par(mfrow=c(2,1))
hist(symb_1wo_3wpe_PAR_data$raw.x)
hist(symb_1wo_3wpe_PAR_data$symbionts)
par(mfrow=c(1,1))
plot(symb_1wo_3wpe_PAR_data$log.x, symb_1wo_3wpe_PAR_data$symbionts) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_lat_symb_1wo_3wpe_PAR_all <- bayesnec::bnec(symbionts ~ crf(log.x, model = "decline"),
                                     data = symb_1wo_3wpe_PAR_data, family = bernoulli(link = "identity"))
save(out_lat_symb_1wo_3wpe_PAR_all, file = "2018.01_AM_HFO_1wo_lat_symb_3wpe_PAR_modfit_all.RData")
...
```{r}
load(file = "2018.01_AM_HFO_1wo_lat_symb_3wpe_PAR_modfit_all.RData")
rhat(out_lat_symb_1wo_3wpe_PAR_all, rhat_cutoff = 1.03)
```


Check model summary
```{r}
out_lat_symb_1wo_3wpe_PAR_all$mod_stats
summary(out_lat_symb_1wo_3wpe_PAR_all)
```


Check the overall MANEC fit
```{r}
plot(out_lat_symb_1wo_3wpe_PAR_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```

Check chain mixing, priors and the fit of the individual candidate models
```{r}
check_chains(out_lat_symb_1wo_3wpe_PAR_all, filename = "2018.01_AM_HFO_1wo_lat_symb_3wpe_PAR_modfit_chains")

check_priors(out_lat_symb_1wo_3wpe_PAR_all, filename = "2018.01_AM_HFO_1wo_lat_symb_3wpe_PAR_modfit_priors")

pdf("2018.01_AM_HFO_1wo_lat_symb_3wpe_PAR_modfit_fitplots.pdf")
plot(out_lat_symb_1wo_3wpe_PAR_all, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
  ...
  *Any indications that the prior distributions were influencing the posterior distributions more than the observed data?*
      - Likely for ecxll5 (ec50 & top)

  *Should any of the candidate models be excluded based on the fit relative to the raw data?*
      - All, no concentration-response observed across the assessed concentrations
...
unique(symb_1wo_3wpe_UV_data$raw.x)
par(mfrow=c(2,1))
hist(symb_1wo_3wpe_UV_data$raw.x)
hist(symb_1wo_3wpe_UV_data$symbionts)
par(mfrow=c(1,1))
plot(symb_1wo_3wpe_UV_data$log.x, symb_1wo_3wpe_UV_data$symbionts) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_lat_symb_1wo_3wpe_UV_all <- bayesnec::bnec(symbionts ~ crf(log.x, model = "decline"),
                                     data = symb_1wo_3wpe_UV_datafamily = bernoulli(link = "identity"))
save(out_lat_symb_1wo_3wpe_UV_all, file = "2018.01_AM_HFO_1wo_lat_symb_3wpe_UV_modfit_all.RData")
...
Check if any candidate models failed the rHat test
```{r}
load(file = "2018.01_AM_HFO_1wo_lat_symb_3wpe_UV_modfit_all.RData")
rhat(out_lat_symb_1wo_3wpe_UV_all, rhat_cutoff = 1.03)
```
...
Check model summary
```{r}
out_lat_symb_1wo_3wpe_UV_all$mod_stats
summary(out_lat_symb_1wo_3wpe_UV_all)
```


Check the overall MANEC fit
```{r}
plot(out_lat_symb_1wo_3wpe_UV_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```

Check chain mixing, priors and the fit of the individual candidate models
```{r}
check_chains(out_lat_symb_1wo_3wpe_UV_all, filename = "2018.01_AM_HFO_1wo_lat_symb_3wpe_UV_modfit_chains")

check_priors(out_lat_symb_1wo_3wpe_UV_all, filename = "2018.01_AM_HFO_1wo_lat_symb_3wpe_UV_modfit_priors")

pdf("2018.01_AM_HFO_1wo_lat_symb_3wpe_UV_modfit_fitplots.pdf")
plot(out_lat_symb_1wo_3wpe_UV_all, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
  ...
  *Should any of the candidate models be excluded based on the fit relative to the raw data?*
      - All, no concentration-response was observed.
...
```{r}
load("2018.01_AM_HFO_1wo_lat_symb_3wpe_data.RData")
...
p.all= p.all+ stat_dots(data = symb_1wo_3wpe_PAR_data, aes(x = raw.x, y = symbionts, colour=factor(light), shape = factor(light), alpha = 0.3, side = ifelse(symbionts == 0, "top", "bottom")), scale = 0.6, position=position_jitter(width = .07, height=0))
p.all= p.all+ stat_dots(data = symb_1wo_3wpe_UV_data, aes(x = raw.x, y = symbionts, colour=factor(light), shape = factor(light), alpha = 0.3, side = ifelse(symbionts == 0, "top", "bottom")), scale = 0.6, position=position_jitter(width = .07, height=0))
...
#p.all= p.all+ geom_point(data = symb_1wo_3wpe_UV_data, aes(x = raw.x, y = symbionts, colour=factor(light), shape = factor(light), alpha = 0.3), size = 2.5, position=position_jitter(width = .025, height=0.03))
