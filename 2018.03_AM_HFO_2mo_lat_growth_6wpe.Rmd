---
title: "2018.03_AM_HFO_2mo_lat_growth_6wpe"
...
growth_2mo_6wpe_data <-juv_lat_growth_data %>% 
  dplyr::filter(exp_time=="6 weeks post-exposure") 

#filter for whether replicates should be included in model fitting
growth_2mo_6wpe_data <- growth_2mo_6wpe_data %>% 
  dplyr::filter(use_in_model=="Yes")



#filter for light treatment
growth_2mo_6wpe_PAR_data <- growth_2mo_6wpe_data %>% 
  dplyr::filter(light=="PAR")
  
growth_2mo_6wpe_UV_data <- growth_2mo_6wpe_data %>% 
  dplyr::filter(light=="UVR")

#check that filtering worked
View(growth_2mo_6wpe_PAR_data)
View(growth_2mo_6wpe_UV_data)
```


### Save filtered data as an RData object
```{r}
save(growth_2mo_6wpe_PAR_data, growth_2mo_6wpe_UV_data, file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_data.RData")
```
...
```{r}
load(file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_data.RData")

unique(growth_2mo_6wpe_PAR_data$raw.x)
par(mfrow=c(2,1))
hist(growth_2mo_6wpe_PAR_data$raw.x)
hist(growth_2mo_6wpe_PAR_data$growth)
par(mfrow=c(1,1))
plot(growth_2mo_6wpe_PAR_data$log.x, growth_2mo_6wpe_PAR_data$growth) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_lat_growth_2mo_6wpe_PAR_all <- bayesnec::bnec(growth ~ crf(log.x, model = "decline"),
                                     data = growth_2mo_6wpe_PAR_data, family = Gamma(link = "identity"))
save(out_lat_growth_2mo_6wpe_PAR_all, file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_PAR_modfit_all.RData")
...
```{r}
load(file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_PAR_modfit_all.RData")
rhat(out_lat_growth_2mo_6wpe_PAR_all, rhat_cutoff = 1.03)
```


Check model summary
```{r}
out_lat_growth_2mo_6wpe_PAR_all$mod_stats
summary(out_lat_growth_2mo_6wpe_PAR_all)
```


Check the overall MANEC fit
```{r}
plot(out_lat_growth_2mo_6wpe_PAR_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```

Check chain mixing, priors and the fit of the individual candidate models
```{r}
check_chains(out_lat_growth_2mo_6wpe_PAR_all, filename = "2018.03_AM_HFO_2mo_lat_growth_6wpe_PAR_modfit_chains")

check_priors(out_lat_growth_2mo_6wpe_PAR_all, filename = "2018.03_AM_HFO_2mo_lat_growth_6wpe_PAR_modfit_priors")

pdf("2018.03_AM_HFO_2mo_lat_growth_6wpe_PAR_modfit_fitplots.pdf")
plot(out_lat_growth_2mo_6wpe_PAR_all, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
  ...
  *Any indications that the prior distributions were influencing the posterior distributions more than the observed data?*
      - Possibly for the ec50 and nec parameters of most models.
...
```{r}
out_lat_growth_2mo_6wpe_PAR_mod <- amend(out_lat_growth_2mo_6wpe_PAR_all, drop = c("ecxll5"))

save(out_lat_growth_2mo_6wpe_PAR_all, out_lat_growth_2mo_6wpe_PAR_mod, file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_PAR_modfit_all.RData")
```


Compare the MANEC containing all OD candidate models to the amended MANEC
```{r}
plot(out_lat_growth_2mo_6wpe_PAR_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
plot(out_lat_growth_2mo_6wpe_PAR_mod, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
out_lat_growth_2mo_6wpe_PAR <- out_lat_growth_2mo_6wpe_PAR_mod
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out_lat_growth_2mo_6wpe_PAR_NEC <- pull_out(out_lat_growth_2mo_6wpe_PAR, model = c("nec"))
```


```{r}
plot(out_lat_growth_2mo_6wpe_PAR_NEC, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out_lat_growth_2mo_6wpe_PAR, out_lat_growth_2mo_6wpe_PAR_NEC, file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_PAR_modfits_final.RData")
load("2018.03_AM_HFO_2mo_lat_growth_6wpe_PAR_modfits_final.RData")
```
...
```{r}
NEC_lat_growth_2mo_6wpe_PAR <- out_lat_growth_2mo_6wpe_PAR_NEC$w_nec
NEC_lat_growth_2mo_6wpe_PAR <- 10^NEC_lat_growth_2mo_6wpe_PAR #if using log10(raw.x)
#NEC_lat_growth_2mo_6wpe_PAR <- exp(NEC_lat_growth_2mo_6wpe_PAR) #if using log(raw.x))
NEC_lat_growth_2mo_6wpe_PAR
```

```{r}
EC10_lat_growth_2mo_6wpe_PAR <- ecx(out_lat_growth_2mo_6wpe_PAR, ecx_val = 10, type = "absolute")
EC10_lat_growth_2mo_6wpe_PAR <- 10^EC10_lat_growth_2mo_6wpe_PAR #if using log10(raw.x)
#EC10_lat_growth_2mo_6wpe_PAR <- exp(EC10_lat_growth_2mo_6wpe_PAR) #if using log(raw.x))
EC10_lat_growth_2mo_6wpe_PAR
```

```{r}
EC50_lat_growth_2mo_6wpe_PAR <- ecx(out_lat_growth_2mo_6wpe_PAR, ecx_val = 50, type = "absolute")
EC50_lat_growth_2mo_6wpe_PAR <- 10^EC50_lat_growth_2mo_6wpe_PAR #if using log10(raw.x)
#EC50_lat_growth_2mo_6wpe_PAR <- exp(EC50_lat_growth_2mo_6wpe_PAR) #if using log(raw.x))
EC50_lat_growth_2mo_6wpe_PAR
```
...
unique(growth_2mo_6wpe_UV_data$raw.x)
par(mfrow=c(2,1))
hist(growth_2mo_6wpe_UV_data$raw.x)
hist(growth_2mo_6wpe_UV_data$growth)
par(mfrow=c(1,1))
plot(growth_2mo_6wpe_UV_data$log.x, growth_2mo_6wpe_UV_data$growth) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_lat_growth_2mo_6wpe_UV_all <- bayesnec::bnec(growth ~ crf(log.x, model = "decline"),
                                     data = growth_2mo_6wpe_UV_data, family = Gamma(link = "identity"))
save(out_lat_growth_2mo_6wpe_UV_all, file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_UV_modfit_all.RData")
...
Check if any candidate models failed the rHat test
```{r}
load(file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_UV_modfit_all.RData")
rhat(out_lat_growth_2mo_6wpe_UV_all, rhat_cutoff = 1.03)
```
...
Check model summary
```{r}
out_lat_growth_2mo_6wpe_UV_all$mod_stats
summary(out_lat_growth_2mo_6wpe_UV_all)
```


Check the overall MANEC fit
```{r}
plot(out_lat_growth_2mo_6wpe_UV_all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```

Check chain mixing, priors and the fit of the individual candidate models
```{r}
check_chains(out_lat_growth_2mo_6wpe_UV_all, filename = "2018.03_AM_HFO_2mo_lat_growth_6wpe_UV_modfit_chains")

check_priors(out_lat_growth_2mo_6wpe_UV_all, filename = "2018.03_AM_HFO_2mo_lat_growth_6wpe_UV_modfit_priors")

pdf("2018.03_AM_HFO_2mo_lat_growth_6wpe_UV_modfit_fitplots.pdf")
plot(out_lat_growth_2mo_6wpe_UV_all, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
  ...
  *Any indications that the prior distributions were influencing the posterior distributions more than the observed data?*
      - Yes, primarily for bot and ec50
...
Rename model output for selected MANEC
```{r}
out_lat_growth_2mo_6wpe_UV <- out_lat_growth_2mo_6wpe_UV_all
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out_lat_growth_2mo_6wpe_UV_NEC <- pull_out(out_lat_growth_2mo_6wpe_UV, model = c("nec"))
```


```{r}
plot(out_lat_growth_2mo_6wpe_UV_NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out_lat_growth_2mo_6wpe_UV, out_lat_growth_2mo_6wpe_UV_NEC, file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_UV_modfits_final.RData")
load("2018.03_AM_HFO_2mo_lat_growth_6wpe_UV_modfits_final.RData")
```
...
Extract NEC, EC10 and EC50 (with 95% credible intervals) from posterior and backtransform if required
```{r}
NEC_lat_growth_2mo_6wpe_UV <- out_lat_growth_2mo_6wpe_UV_NEC$w_nec
NEC_lat_growth_2mo_6wpe_UV <- 10^NEC_lat_growth_2mo_6wpe_UV #if using log10(raw.x)
#NEC_lat_growth_2mo_6wpe_UV <- exp(NEC_lat_growth_2mo_6wpe_UV) #if using log(raw.x))
NEC_lat_growth_2mo_6wpe_UV
```

```{r}
EC10_lat_growth_2mo_6wpe_UV <- ecx(out_lat_growth_2mo_6wpe_UV, ecx_val = 10, type = "absolute")
EC10_lat_growth_2mo_6wpe_UV <- 10^EC10_lat_growth_2mo_6wpe_UV #if using log10(raw.x)
#EC10_lat_growth_2mo_6wpe_UV <- exp(EC10_lat_growth_2mo_6wpe_UV) #if using log(raw.x))
EC10_lat_growth_2mo_6wpe_UV
```

```{r}
EC50_lat_growth_2mo_6wpe_UV <- ecx(out_lat_growth_2mo_6wpe_UV, ecx_val = 50, type = "absolute")
EC50_lat_growth_2mo_6wpe_UV <- 10^EC50_lat_growth_2mo_6wpe_UV #if using log10(raw.x)
#EC50_lat_growth_2mo_6wpe_UV <- exp(EC50_lat_growth_2mo_6wpe_UV) #if using log(raw.x))
EC50_lat_growth_2mo_6wpe_UV
```
...
pred_lat_growth_2mo_6wpe <- compare_posterior(x=list("PAR" = out_lat_growth_2mo_6wpe_PAR, "UVR" = out_lat_growth_2mo_6wpe_UV), comparison = "fitted", precision = 50)

pred_lat_growth_2mo_6wpe_PAR <- pred_lat_growth_2mo_6wpe$posterior_data %>% 
  dplyr::filter(model=="PAR")

pred_lat_growth_2mo_6wpe_UV <- pred_lat_growth_2mo_6wpe$posterior_data %>% 
  dplyr::filter(model=="UVR")



pred_lat_growth_2mo_6wpe_NEC <- compare_posterior(list("PAR" = out_lat_growth_2mo_6wpe_PAR_NEC, "UVR" = out_lat_growth_2mo_6wpe_UV_NEC), comparison = "fitted", precision = 50)

pred_lat_growth_2mo_6wpe_PAR_NEC <- pred_lat_growth_2mo_6wpe_NEC$posterior_data %>% 
  dplyr::filter(model=="PAR")
pred_lat_growth_2mo_6wpe_UV_NEC <- pred_lat_growth_2mo_6wpe_NEC$posterior_data %>% 
 dplyr::filter(model=="UVR")


save(pred_lat_growth_2mo_6wpe, pred_lat_growth_2mo_6wpe_PAR, pred_lat_growth_2mo_6wpe_UV, pred_lat_growth_2mo_6wpe_PAR_NEC, pred_lat_growth_2mo_6wpe_UV_NEC, file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_predvals.RData") # pred_lat_growth_2mo_6wpe_NEC, pred_lat_growth_2mo_6wpe_PAR_NEC, pred_lat_growth_2mo_6wpe_UV_NEC <- add these back in once UV-section is working

load("2018.03_AM_HFO_2mo_lat_growth_6wpe_predvals.RData")
str(pred_lat_growth_2mo_6wpe_PAR)
str(pred_lat_growth_2mo_6wpe_UV)
```

## Extract the median repsonse for controls and unaffected treatment concnetrations
```{r}
head(pred_lat_growth_2mo_6wpe_PAR)
head(pred_lat_growth_2mo_6wpe_UV)
```

```{r}
tail(pred_lat_growth_2mo_6wpe_PAR)
tail(pred_lat_growth_2mo_6wpe_UV)
```
...
diff_lat_growth_2mo_6wpe <- max(pred_lat_growth_2mo_6wpe$prob_diff$prob)
diff_lat_growth_2mo_6wpe
```
...
comp_lat_growth_2mo_6wpe_EC50 <- compare_posterior(x=list("PAR" = out_lat_growth_2mo_6wpe_PAR, "UVR" = out_lat_growth_2mo_6wpe_UV), comparison = "ecx", ecx_val = 50)
save(comp_lat_growth_2mo_6wpe_EC50, file = "2018.03_AM_HFO_2mo_lat_growth_6wpe_posterior_comp_EC50.RData")
comp_lat_growth_2mo_6wpe_EC50$prob_diff
```
...
```{r}
load("2018.03_AM_HFO_2mo_lat_growth_6wpe_data.RData")
load("2018.03_AM_HFO_2mo_lat_growth_6wpe_predvals.RData")
...
p.all= p.all+ geom_point(data = growth_2mo_6wpe_UV_data, aes(x = raw.x, y = growth, colour=factor(light), shape = factor(light), alpha = 0.3), size = 2.5, position=position_jitter(width = .025, height=0.03))


p.all = p.all+ labs(x=expression(Concentration~(μg~TAH~"L"^{-1})), #Concentration~(μg~TAH~"L"^{-1})
              y=expression(Relative~growth~("%")),
              subtitle="a)")
p.all= p.all+ scale_y_continuous( limits = c(-0.03, 27.55), breaks = c(0, 0.25, 0.5, 0.75, 1.0), labels = c(0, 25, 50, 75, 100)) 
...
p_NEC= p_NEC+ geom_point(data = growth_2mo_6wpe_UV_data, aes(x = raw.x, y = growth, colour=factor(light), shape = factor(light), alpha = 0.3), size = 2.5, position=position_jitter(width = .025, height=0.03))
...
pred_lat_growth_2mo_6wpe$prob_diff$inv.x <- 10^pred_lat_growth_2mo_6wpe$prob_diff$x

p.comp.prob= ggplot(pred_lat_growth_2mo_6wpe$prob_diff) +
...
plot_lat_growth_2mo_6wpe_diff.prob <- p.comp.prob
plot_lat_growth_2mo_6wpe_diff.prob
```
...
```{r}
load("2018.03_AM_HFO_2mo_lat_growth_6wpe_posterior_comp_EC50.RData")

p.comp.EC50 = ggplot(comp_lat_growth_2mo_6wpe_EC50$posterior_data, aes(x=value))+geom_density(aes(group=model, color=model, fill=model), alpha=0.4) 
...
p.comp.EC50= p.comp.EC50+ annotate("text", label = "UVR < PAR = 9.2%", x = 0.55, y = 7.86, size = 2.4, colour = "grey20") #0.906 of the first annotation


#Rename & show plot
comp.plot_lat_growth_2mo_6wpe_EC50 <- p.comp.EC50
comp.plot_lat_growth_2mo_6wpe_EC50
```
...
p.comp.EC50.diff = ggplot(comp_lat_growth_2mo_6wpe_EC50$diff_data, aes(x=diff)) +
...
#Rename & show plot
comp.plot_lat_growth_2mo_6wpe_EC50.diff <- p.comp.EC50.diff
comp.plot_lat_growth_2mo_6wpe_EC50.diff
```
...
```{r}
layout_HFO_2mo_lat_growth_6wpe <- ggarrange(plot_lat_growth_2mo_6wpe_all, comp.plot_lat_growth_2mo_6wpe_EC50, plot_lat_growth_2mo_6wpe_diff.est, plot_lat_growth_2mo_6wpe_NEC,
          ncol = 3, nrow = 2,
          widths = c(0.5, 0.5))
layout_HFO_2mo_lat_growth_6wpe
```
