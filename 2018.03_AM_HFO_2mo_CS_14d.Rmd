---
title: "2018.03_AM_HFO_2mo_CS_14d_v1"
...
```{r}
save(CS_14d_PAR_data, CS_14d_UV_data, file = "2018.03_AM_HFO_2mo_CS_14d_data.RData")
```
...
```{r}
load(file = "2018.03_AM_HFO_2mo_CS_14d_data.RData")

unique(CS_14d_PAR_data$raw.x)
par(mfrow=c(2,1))
hist(CS_14d_PAR_data$raw.x)
hist(CS_14d_PAR_data$CS)
par(mfrow=c(1,1))
plot(CS_14d_PAR_data$log.x, CS_14d_PAR_data$CS) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_CS_14d_PAR_all <- bayesnec::bnec(CS ~ crf(log.x, model = "decline"),
                                     data = CS_14d_PAR_data)
save(out_CS_14d_PAR_all, file = "2018.03_AM_HFO_2mo_CS_14d_PAR_modfit_all.RData")
beepr::beep("mario")
```
...
unique(CS_14d_UV_data$raw.x)
par(mfrow=c(2,1))
hist(CS_14d_UV_data$raw.x)
hist(CS_14d_UV_data$CS)
par(mfrow=c(1,1))
plot(CS_14d_UV_data$log.x, CS_14d_UV_data$CS) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
out_CS_14d_UV_all <- bayesnec::bnec(CS ~ crf(log.x, model = "decline"),
                                     data = CS_14d_UV_data)
save(out_CS_14d_UV_all, file = "2018.03_AM_HFO_2mo_CS_14d_UV_modfit_all.RData")
beepr::beep("mario")
```
...
CS_0d_data <- CS_0d_data %>% 
  dplyr::filter(use_in_plot=="Yes")

#filter for light treatment
CS_0d_PAR_data <- CS_0d_data %>% 
  dplyr::filter(light=="PAR")
  
CS_0d_UV_data <- CS_0d_data %>% 
  dplyr::filter(light=="UVR")

#replace PAR and UVR with PAR_t0 and UVR_t0
CS_0d_PAR_data <- CS_0d_PAR_data %>% 
  dplyr::mutate(light="PAR_t0")
  
CS_0d_UV_data <- CS_0d_UV_data %>% 
...
save(CS_14d_PAR_data, CS_14d_UV_data, CS_14d_HT_data, file = "2018.03_AM_HFO_2mo_CS_14d_data.RData")
save(CS_0d_PAR_data, CS_0d_UV_data, CS_0d_HT_data, file = "2018.03_AM_HFO_2mo_CS_0d_data.RData")
```



```{r}
load("2018.03_AM_HFO_2mo_CS_14d_data.RData")
...
p.all= p.all+ geom_point(data = CS_14d_UV_data, aes(x = raw.x, y = CS, colour=factor(light), shape = factor(light), alpha = 0.3), size = 2.5, position=position_jitter(width = .035, height=0.03))
...
p.all= p.all+ scale_x_continuous(limits = c(0.09, 1000), trans="log10", breaks = c(0.01, 1, 10, 100, 1000), labels = c(0, 1, 10, 100, 1000))
